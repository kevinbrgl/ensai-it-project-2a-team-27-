...
Le schéma de la base de données s'articule autour de deux entités : les membres et les œuvres littéraires. La table `USERS` est conçue pour garantir l'intégrité des données d'identification. Notamment par l'application de contraintes d'unicité sur le pseudonyme et l'adresse électronique, tout en prévoyant le stockage du mot de passe haché et des éléments de profil. En parallèle, la table `BOOKS` permet de répertorier des œuvres, stockant les données utilent à l'application telles que le titre, l'auteur, la catégorie et la date de publication.
...
Au-delà du fonctionnel décrit précédemment, nous avons identifié plusieurs pistes d'amélioration qui ne sont pas indispensables au bon fonctionnement de l'application. Cepepdant elles nous semblent pertinentes pour enrichir l'expérience utilisateur si le temps du projet nous le permet. Ces fonctionnalités ne conditionnent pas la livraison du projet, elles représentent plutôt ce que nous ajouterions si Ex-Libris devait réellement être proposée à des utilisateurs.
